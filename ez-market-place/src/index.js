/**
 * EZ-Cloud - Market Place Backend
 *
 * @summary Main file for service
 * @author Tony Massé
 *
 * Created at     : 2022-02-01 21:30:00
 * Last modified  : 2022-02-01 21:30:00
 */

const path = require('path');
require('dotenv').config();

// Storing the base directory name of the process, to be used elsewere
// while loading configuration and other files
// The reason for this is that once packed, all these calls are made from
// the very same file, so __dirname of source
...
process.env.NAME = 'EZ-Cloud - Market Place Backend';
process.env.VERSION = version;

logToSystem('Information', `${process.env.NAME} - Version: ${process.env.VERSION} - Started`, true);

httpServer.listen(port, host, () => {
  logToSystem('Information', `Listening for HTTP requests on: http://${host}:${port}`, true);
});
