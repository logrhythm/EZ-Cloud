...
function logHttpToSystem(req, res, next) {
  logToSystem('Verbose', `HTTP Request | client_ip: ${(req.socket && req.socket._peername && req.socket._peername.address ? req.socket._peername.address : '-')} | client_port: ${(req.socket && req.socket._peername && req.socket._peername.port ? req.socket._peername.port : '-')} | remote_addr: ${(req.headers && req.headers['x-forwarded-for'] ? req.headers['x-forwarded-for'] : '-')} | deployment_uid: ${(req.ezPublisherHeader && req.ezPublisherHeader.deploymentUid ? req.ezPublisherHeader.deploymentUid : '-')} | deployment_master_id: ${(req.ezPublisherHeader && req.ezPublisherHeader.masterId ? req.ezPublisherHeader.masterId : '-')} | publisher_uid: ${(req.ezPublisherHeader && req.ezPublisherHeader.publisherUid ? req.ezPublisherHeader.publisherUid : '-')} | server_version: ${(req.ezVersions && req.ezVersions.server ? req.ezVersions.server : '-')} | client_version: ${(req.ezVersions && req.ezVersions.client ? req.ezVersions.client : '-')} | method: ${(req.method ? req.method : '-')} | path: ${(req.url ? req.url : '-')}`);
  next();
}
