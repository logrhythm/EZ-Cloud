 ...
 */
export class Step1Validator extends Validator {
  /**
  ...
  static validatePolicyStructure (policy) {
    ...
    // Normalize schemarule variants: accept schemarule, schemaRule, schemarules, etc.
    ...
    // Validate schemarule structure if present (now normalized)
  ...
  static sanitizeAndExtractJson (text) {
    ...
    // Prefer the shared sanitizer if available
    let cleaned = ''
    if (ValidationUtils && typeof ValidationUtils.sanitizeJsonText === 'function') {
      cleaned = ValidationUtils.sanitizeJsonText(text)
    } else {
      // fallback to existing comment-stripping + trailing-comma removal
      cleaned = this.stripCommentsPreserveStrings(text)
  ...
  static validateJsonContent (content) {
    ...
    try {
      const jsonText = this.sanitizeAndExtractJson(content)
      ...
      // Primary attempt failed — try fallbacks to be forgiving in UI
      try {
        // Fallback 1: explicit comment strip + extract
        const withoutComments = this.stripCommentsPreserveStrings(content)
        const cleaned = withoutComments.replace(/,\s*(\}|\])/g, '$1')
        const extracted = this.extractJsonFromText(cleaned) || cleaned
        const parsedFallback = JSON.parse(extracted)
        ...
        try {
          // Fallback 2: aggressively remove any leading // lines before first '{'
          const idx = content.indexOf('{')
          const prefix = idx >= 0 ? content.slice(0, idx) : ''
          const suffix = idx >= 0 ? content.slice(idx) : content
          const prefixStripped = prefix.replace(/\/\/.*?(\r?\n|$)/g, '')
          const aggressive = (prefixStripped + suffix).replace(/,\s*(\}|\])/g, '$1')
          const finalExtract = this.extractJsonFromText(aggressive) || aggressive
          const parsedAggressive = JSON.parse(finalExtract)

          if (typeof parsedAggressive !== 'object' || parsedAggressive === null) {
            result.addError('content', 'JSON must be an object, not a primitive value')
          }

          return result
        } catch (err3) {
          // All attempts failed — give informative message
          const sample = String(content).slice(0, 200)
          result.addError('content', `Invalid JSON format: ${error && error.message ? error.message : String(error)}. Attempts to auto-clean failed.`)
          result.addInfo('content', `Preview of file start: ${sample.replace(/\n/g, '\\n').replace(/\r/g, '')}`)
          return result
        }
      }
    }
  ...
  /**
   * Remove single-line (//...) and multi-line (/* ... *\/) comments from text while
   * preserving string literals. Returns cleaned text.
   */
