...
BEGIN
  ...
  THEN
    RAISE NOTICE '% - Table "settings" already exists. Skipping.', clock_timestamp()::TEXT;
  ELSE
    ...
    RAISE NOTICE '% - Table "settings" succesfully created.', clock_timestamp()::TEXT;
  END IF;

  RAISE NOTICE '% - Update table "settings" ownership and indices, if necessary.', clock_timestamp()::TEXT;

  ALTER TABLE IF EXISTS public."settings"
    OWNER to "oc-admin-backend";

  RAISE NOTICE '% - Update table "settings" content: Default content', clock_timestamp()::TEXT;
  ...
  RAISE NOTICE '% - Table "settings" content updated.', clock_timestamp()::TEXT;
